...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const clicked = await page.evaluate(() => {
      // Find avatar button - it's an image with dropdown
      const avatarButtons = Array.from(document.querySelectorAll('button, div[role="button"]'));
      for (const btn of avatarButtons) {
        const img = btn.querySelector('img[class*="avatar"], img[alt*="avatar"]');
        if (img) {
          btn.click();
          return true;
        }
