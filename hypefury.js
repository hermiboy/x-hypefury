      ...
      for (const tweet of selectedTweets) {
        ...
        try {
          ...
          try {
            ...
            // CRITICAL: Manually trigger input event to enable Reply button
            await page.evaluate((el, selector) => {
              const textarea = el.querySelector(selector);
              if (textarea) {
                textarea.dispatchEvent(new Event('input', { bubbles: true }));
                console.log('[BROWSER] Input event triggered manually');
              }
            }, tweetEl, textareaSelector);
            
            await sleep(gaussianRandom(1500, 2500));
            
            // Click Reply button - find ENABLED button only
            ...
            const replyClicked = await page.evaluate(() => {
              ...
              if (enabledBtn) {
                console.log('[BROWSER] Found enabled button, clicking...');
                enabledBtn.click();
                return true;
              } else {
                console.log('[BROWSER] No enabled button found');
                return false;
              }
            });
