...
async function switchAccount(page, targetHandle) {
  try {
    ...
    await page.waitForSelector('button.btn.dropdown-item, button[class*="dropdown"]', { timeout: 5000 }).catch(() => {});
    
    const accountClicked = await page.evaluate((targetName) => {
      const dropdownButtons = Array.from(document.querySelectorAll('button.btn.dropdown-item, button[class*="dropdown"]'));
...
async function doLike(page, tweetEl, accountState) {
  ...
  try {
    ...
    const likeResult = await page.evaluate(() => {
      const likeBtn = document.querySelector('button[name="Like"]');
      ...
      const beforeState = likeBtn.getAttribute('aria-pressed') || 'false';
      console.log('[BROWSER] Before state:', beforeState);
      
      likeBtn.click();
      console.log('[BROWSER] Click executed');
      
      return new Promise((resolve) => {
        setTimeout(() => {
          const afterState = likeBtn.getAttribute('aria-pressed') || 'false';
          console.log('[BROWSER] After state:', afterState);
          
          if (beforeState !== afterState) {
            resolve({ success: true, before: beforeState, after: afterState });
          } else {
            resolve({ success: false, reason: 'no_state_change', before: beforeState, after: afterState });
          }
        }, 1000);
      });
    });
    ...
    if (likeResult.success) {
      ...
      console.log('💙 Liked! (verified state change)');
      return true;
    } else {
      console.log(`⚠️ Like failed: ${likeResult.reason}`, likeResult);
      return false;
    }
      ...
      for (const tweet of selectedTweets) {
        ...
        try {
          ...
          try {
            ...
            // Find and click textarea within this tweet's context
            console.log('🔍 DEBUG: Looking for textarea...');
            const textareaClicked = await page.evaluate((el, selector) => {
              const textarea = el.querySelector(selector);
              console.log('[BROWSER] Textarea found:', !!textarea);
              if (textarea) {
                textarea.click();
                textarea.focus();
                return true;
            ...
            if (!textareaClicked) {
              console.log('⚠️ Textarea not found in tweet');
              continue;
            }
            console.log('✅ DEBUG: Textarea clicked and focused');
            ...
            await sleep(gaussianRandom(1200, 2000));
            
            // Click Reply button - search GLOBALLY since it's outside tweet context
            console.log('🔍 DEBUG: Looking for Reply button...');
            const replyClicked = await page.evaluate(() => {
              const buttons = document.querySelectorAll('button[data-cy="new-feed-reply"]');
              console.log('[BROWSER] Reply buttons found:', buttons.length);
              
              for (const btn of buttons) {
                const rect = btn.getBoundingClientRect();
                const isVisible = rect.width > 0 && rect.height > 0 && 
                                 rect.top >= 0 && rect.bottom <= window.innerHeight;
                console.log('[BROWSER] Button visible:', isVisible, 'disabled:', btn.disabled);
                
                if (isVisible && !btn.disabled) {
                  btn.click();
                  console.log('[BROWSER] Reply button clicked!');
                  return true;
                }
              }
              return false;
            ...
            if (replyClicked) {
              ...
              console.log('⚠️ Reply button not found or not clickable');
            }
