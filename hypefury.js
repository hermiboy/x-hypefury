...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const accountClicked = await page.evaluate((targetName) => {
      ...
      const dropdownButtons = Array.from(document.querySelectorAll('button.btn-dropdown, button.dropdown-item'));
      
      for (const btn of dropdownButtons) {
        const text = btn.textContent.trim();
        const ariaLabel = btn.getAttribute('aria-label') || '';
        
        // Match by text content or aria-label containing target name
