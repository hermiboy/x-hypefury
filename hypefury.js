...
async function doLike(page, tweetEl, accountState) {
  ...
  try {
    ...
    const likeResult = await page.evaluate(() => {
      ...
      if (!likeBtn || likeBtn.disabled) return { success: false, reason: 'button_not_found' };
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          if (beforeState !== afterState) {
            ...
            resolve({ success: false, reason: 'no_state_change' });
          }
        }, 1000);
      });
    });
    ...
    for (const accountConfig of CONFIG.accounts) {
      ...
      for (const tweet of selectedTweets) {
        ...
        try {
          ...
          const textareaSelector = 'textarea[placeholder="What would you like to say?"]';
          try {
            const tweetEl = await findTweetElement(page, tweet.id);
            if (!tweetEl) {
              console.log('⚠️ Tweet element not found');
            ...
            // Wait for Reply button to become enabled
            const replyBtnReady = await page.evaluate((el) => {
              const btn = el.querySelector('button[data-cy="new-feed-reply"]');
              if (!btn) return false;
            ...
            if (!replyBtnReady) {
              console.log('⚠️ Reply button did not become enabled');
              continue;
            }
            ...
            if (replyClicked) {
              ...
              console.log('⚠️ Reply button click failed');
            }
