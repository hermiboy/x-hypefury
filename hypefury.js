...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const currentAccount = await page.evaluate(() => {
      ...
      const selectors = [
        'button[class*="avatar"] span',
        'div[class*="avatar"] span',
        '[class*="account"] span',
        'button:has(img[class*="avatar"]) span',
        'div:has(img[class*="avatar"]) span'
      ];
      ...
      for (const selector of selectors) {
        ...
        if (el) {
          ...
          // Filter out follower counts and other numbers
      ...
      // Fallback: look for visible text near avatar
      const avatarImg = document.querySelector('img[class*="avatar"]');
      if (avatarImg) {
        const parent = avatarImg.closest('button, div');
        if (parent) {
          // Get all text nodes
          ...
          while (node = walker.nextNode()) {
            ...
            if (text && text.length > 0 && text.length < 50 && !/^\d+$/.test(text) && text !== 'avatar') {
              return text;
            }
          }
        }
    ...
    if (currentAccount && currentAccount === targetName) {
      console.log(`✅ Already on ${targetHandle}`);
      return true;
    }
    
    // Click avatar/dropdown to open menu
    const avatarBtn = await page.$('button[class*="avatar"], div[class*="avatar"]');
    if (!avatarBtn) {
      console.log('⚠️ Avatar button not found');
      return false;
    }
    ...
    // Wait for dropdown to be visible and find account in it
    const switched = await page.evaluate((targetName) => {
      // Wait a bit for dropdown animation
      return new Promise((resolve) => {
        setTimeout(() => {
          // Look for elements that appeared recently (dropdown items)
          const allElements = Array.from(document.querySelectorAll('button, div[role="button"], a, div'));
          
          for (const el of allElements) {
            const text = el.textContent.trim();
            
            // Match exact name or name with checkmark/icon
            if (text === targetName || text.startsWith(targetName)) {
              // Make sure it's visible (in dropdown)
              const rect = el.getBoundingClientRect();
              if (rect.width > 0 && rect.height > 0) {
                console.log(`[SWITCH] Found "${targetName}" - clicking`);
                el.click();
                resolve(true);
                return;
              }
          ...
          console.log(`[SWITCH] Could not find "${targetName}" in visible dropdown`);
          resolve(false);
        }, 500);
      });
    }, targetName);
    ...
    console.log(`📍 After switch: ${newAccount}`);
    
    if (newAccount && newAccount === targetName) {
      console.log(`✅ Successfully switched to ${targetHandle} (${targetName})`);
      return true;
    }
    
    console.log(`⚠️ Switch verification failed. Current: ${newAccount}, Expected: ${targetName}`);
    return false;
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          try {
            ...
            if (text.length > 10) {
              ...
              console.log(`[EXTRACT] Tweet ${i}: author="${author}", likes=${likes}, text="${text.substring(0, 60)}..."`);
              results.push({ 
                author, 
                text, 
                timestamp, 
