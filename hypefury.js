...
async function switchAccount(page, targetHandle) {
  try {
    ...
    // CRITICAL: Wait for buttons to load in dropdown
    await sleep(3000);
    
    // Check if buttons loaded
    const buttonsLoaded = await page.evaluate(() => {
      const buttons = document.querySelectorAll('button.btn-dropdown');
      return buttons.length;
    });
    
    console.log(`🔍 Buttons loaded after 3s: ${buttonsLoaded}`);
    
    if (buttonsLoaded === 0) {
      console.log('⚠️ No buttons loaded yet, waiting 3 more seconds...');
      await sleep(3000);
    }
    ...
    const accountClicked = await page.evaluate((targetName) => {
      console.log('[BROWSER DEBUG] ========== ACCOUNT SWITCH DEBUG ==========');
      console.log('[BROWSER DEBUG] Looking for account:', targetName);
      
      const dropdownButtons = document.querySelectorAll('button.btn-dropdown');
      console.log('[BROWSER DEBUG] Total dropdown buttons found:', dropdownButtons.length);
      
      // Log ALL buttons
      console.log('[BROWSER DEBUG] All button texts:');
      Array.from(dropdownButtons).forEach((btn, i) => {
        const text = btn.textContent.trim();
        const rect = btn.getBoundingClientRect();
        console.log(`[BROWSER DEBUG]   ${i}: "${text}" (w=${rect.width}, h=${rect.height})`);
      });
      ...
      for (const btn of dropdownButtons) {
        ...
        console.log(`[BROWSER DEBUG] Comparing: "${text}" === "${targetName}" ? ${text === targetName}`);
        
        // EXACT match only
        if (text === targetName) {
          const rect = btn.getBoundingClientRect();
          console.log(`[BROWSER DEBUG] MATCH FOUND! Size: ${rect.width}x${rect.height}`);
          
          if (rect.width > 0 && rect.height > 0) {
            foundButton = btn;
            break;
          } else {
            console.log('[BROWSER DEBUG] But button has no size!');
          }
        }
      }
      
      if (foundButton) {
        console.log('[BROWSER DEBUG] Clicking matched button...');
        foundButton.click();
        console.log('[BROWSER DEBUG] Click executed!');
        return true;
      } else {
        console.log('[BROWSER DEBUG] NO MATCH FOUND! Account not switched.');
        return false;
      }
    }, targetName);
    ...
    // CRITICAL: Close dropdown after switch to prevent broken state
    await page.evaluate(() => {
      document.body.click(); // Click outside to close dropdown
    });
    await sleep(1000);
...
async function runSession() {
  ...
  try {
    ...
    for (const accountConfig of CONFIG.accounts) {
      ...
      // Extra wait between accounts for page to stabilize
      const accountIndex = CONFIG.accounts.findIndex(a => a.twitter_handle === handle);
      if (accountIndex < CONFIG.accounts.length - 1) {
        console.log('⏳ Waiting 5s before switching to next account...');
        await sleep(5000);
      }
