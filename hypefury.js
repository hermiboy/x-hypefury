...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const handleNoAt = targetHandle.replace('@', '').trim();
    
    // Try dropdown first
    const dropdown = await page.$('button[data-cy="account-dropdown"], button[class*="dropdown"]');
    if (dropdown) {
      await dropdown.click();
      await sleep(gaussianRandom(500, 1000));
    }
    
    // Try XPath for visible buttons containing handle
    const xpath = `//button[contains(normalize-space(.), '${handleNoAt}')]`;
    const els = await page.$x(xpath);
    if (els && els.length) {
      await els[0].click();
      await sleep(gaussianRandom(1000, 2000));
      console.log(`✅ Switched to ${targetHandle}`);
      return true;
    }
    ...
    for (const btn of buttons) {
      ...
      if (txt.trim().includes(handleNoAt)) {
        await btn.click();
        await sleep(gaussianRandom(1000, 2000));
        console.log(`✅ Switched to ${targetHandle}`);
        return true;
    ...
    console.log(`⚠️ Account ${targetHandle} not found`);
    return false;
  } catch (err) {
