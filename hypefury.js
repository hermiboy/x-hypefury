...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const avatarBtn = await page.$('button[class*="avatar"], img[class*="avatar"], div[class*="avatar"]');
    if (avatarBtn) {
    ...
    // Find account by display name in dropdown
    ...
    // Verify switch by checking current account name
    const currentName = await page.evaluate(() => {
      // Check for active account indicator
      const activeBtn = document.querySelector('button[aria-selected="true"], div[class*="active"]');
    ...
    if (currentName && currentName.includes(targetName)) {
      console.log(`✅ Switched to ${targetHandle} (${targetName})`);
      return true;
    }
    
    console.log(`⚠️ Switch verification failed. Current: ${currentName}, Expected: ${targetName}`);
    return false;
