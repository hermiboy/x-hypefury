...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const currentAccount = await page.evaluate(() => {
      const avatarArea = document.querySelector('[class*="avatar"]');
      if (avatarArea) {
        const parent = avatarArea.closest('button, div');
        if (parent) {
          const text = parent.textContent.trim();
          // Extract just the name (before any follower count)
      ...
      const tweets = await page.evaluate(() => {
        ...
        for (const item of items) {
          try {
            ...
            if (text.length > 10) {
              console.log(`[EXTRACT] Tweet: author="${author}", likes=${likes}, text="${text.substring(0, 60)}..."`);
              results.push({ 
                author, 
                text, 
                timestamp, 
            ...
            console.log(`[EXTRACT] Error parsing item:`, e.message);
          }
        }
