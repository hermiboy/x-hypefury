...
ensureChromeNotRunning().then(() => {
  ...
  if (!GROK_API_KEY) {
    ...
    console.log('$env:GROK_API_KEY="xai-YOUR-KEY-HERE"');
  }
...
async function clickThreeDotsMenu(page, tweetEl) {
  try {
    // Find 3-dots button (SVG icon or button with ellipsis)
    const threeDotsClicked = await page.evaluate((el) => {
      // Look for SVG with class that contains "fill-current"
      const svgs = el.querySelectorAll('svg.fill-current');
      ...
      // Fallback: look for button with "..." text or aria-label containing "more"
...
async function switchAccount(page, targetHandle) {
  try {
    ...
    // Map handle to display name (from screenshots)
    const displayNames = {
    ...
    const clicked = await page.evaluate(() => {
      // Look for button containing profile picture (img or avatar)
      const buttons = document.querySelectorAll('button');
      for (const btn of buttons) {
        const img = btn.querySelector('img.h-6.w-6, img[class*="avatar"]');
        if (img) {
          // Check if button has profile-related class
    ...
    // Find and click target account in dropdown
    const accountClicked = await page.evaluate((targetName) => {
      // Look for dropdown buttons with the target name
...
async function doLike(page, tweetEl, accountState) {
  ...
  try {
    ...
    if (!opened) {
      console.log('⚠️ Could not open 3-dots menu for like');
      return false;
    }
    
    // Click Like button in dropdown
    ...
    const tweets = await page.evaluate(() => {
      ...
      for (const item of items) {
        try {
          ...
          const statsMatch = allText.match(/(\d+)\s+(\d+)\s+(\d{1,2}:\d{2}\s(?:AM|PM))/);
          let likes = 0;
          if (statsMatch) {
    ...
    for (const accountConfig of CONFIG.accounts) {
      ...
      const tweets = await page.evaluate(() => {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          try {
            ...
            // Extract author - look for a.avatar-username or @handle
            let author = 'Unknown';
            const usernameLink = item.querySelector('a.avatar-username');
            if (usernameLink) {
              author = usernameLink.textContent.trim();
            } else {
              // Fallback: search for @handle in text
              const handleMatch = item.textContent.match(/@[a-zA-Z0-9_]+/);
              if (handleMatch) {
      ...
      for (let i = 0; i < preLikes && accountState.likesMade < accountState.likeTarget; i++) {
        ...
        if (DRY_RUN) {
          ...
          const tweetEl = await findTweetElement(page, randomTweet.text);
          if (tweetEl) {
            await doLike(page, tweetEl, accountState);
          }
        }
      }
      ...
      for (const tweet of selectedTweets) {
        ...
        try {
          ...
          const textareaSelector = 'textarea[name="What would you like to say?"]';
          try {
            ...
            // Click Reply button
            const replyBtnClicked = await page.evaluate(() => {
              const replyBtn = document.querySelector('button[name="Reply"]');
              if (replyBtn) {
                replyBtn.click();
                return true;
              }
