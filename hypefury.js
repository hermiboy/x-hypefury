...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const clicked = await page.evaluate(() => {
      // First try: specific button with dropdown class
      const dropdownBtn = document.querySelector('button.btn-dropdown.btn-highlight');
      if (dropdownBtn) {
        dropdownBtn.click();
        return true;
      }
      
      // Fallback: find any button with avatar img inside
      const buttons = Array.from(document.querySelectorAll('button, div[role="button"]'));
      for (const btn of buttons) {
        const img = btn.querySelector('img.avatar-img, img.avatar-img-xs');
        if (img) {
          btn.click();
          return true;
        }
    ...
    const accountClicked = await page.evaluate((targetName) => {
      // Dropdown is now open - find account by exact text match
      const allElements = Array.from(document.querySelectorAll('*'));
      
      for (const el of allElements) {
        // Check if element has exactly the target name as text
        const text = el.textContent.trim();
        
        // Must match exactly or be part of a small container
        if (text === targetName || (text.length < 50 && text.includes(targetName))) {
          // Make sure it's clickable and visible
          const rect = el.getBoundingClientRect();
          if (rect.width > 0 && rect.height > 0) {
            // Check if it's in a list/dropdown (has siblings or parent with role)
            const parent = el.closest('[role="menu"], [role="listbox"], div[class*="menu"], div[class*="dropdown"]');
            if (parent || el.closest('button') || el.tagName === 'BUTTON') {
              const clickTarget = el.closest('button') || el;
              clickTarget.click();
              console.log(`[SWITCH] Clicked "${targetName}"`);
              return true;
            }
          }
        }
      ...
      console.log(`[SWITCH] Could not find clickable "${targetName}"`);
      return false;
    }, targetName);
