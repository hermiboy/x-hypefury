...
async function switchAccount(page, targetHandle) {
  try {
    ...
    const accountClicked = await page.evaluate((targetName) => {
      // Search in ALL buttons after dropdown opened
      ...
      for (const btn of dropdownButtons) {
        ...
        // Skip if this is the currently active account (has btn-highlight)
        if (btn.classList.contains('btn-highlight')) {
          console.log(`[SWITCH] Skipping active account: "${text}"`);
          continue;
        }
        
        // Match by text content (primary), aria-label, or name attribute
        // Check text first since aria and name can be null
        const textMatch = text.includes(targetName);
        const ariaMatch = ariaLabel && ariaLabel.toLowerCase().includes(targetName.toLowerCase());
        const nameMatch = name && name.toLowerCase().includes(targetName.toLowerCase());
        
        if (textMatch || ariaMatch || nameMatch) {
          const rect = btn.getBoundingClientRect();
          if (rect.width > 0 && rect.height > 0) {
            // Make sure it's visible (in viewport)
            const isVisible = rect.top >= 0 && rect.left >= 0 && rect.top < window.innerHeight;
            if (isVisible) {
              console.log(`[SWITCH] Found "${targetName}" - Text: "${text}" - Clicking...`);
              btn.click();
              return true;
            }
          }
...
async function runSession() {
  ...
  try {
    ...
    for (const accountConfig of CONFIG.accounts) {
      ...
      // Calculate appropriate session target based on daily goal
      // Expect 3-4 sessions per day, so divide target accordingly
      ...
      const tweets = await page.evaluate(() => {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          try {
            ...
            // Find likes and retweets - pattern: "11  0  2:27 PM"
            // First number = likes, second number = retweets, then time
            let likes = 0;
            let retweets = 0;
      ...
      for (const tweet of selectedTweets) {
        ...
        try {
          ...
          let replyBtn = await tweetEl.$('button[name="Reply"]');
          if (!replyBtn) {
            replyBtn = await tweetEl.$('button.btn-primary:has-text("Reply")');
          }
          if (!replyBtn) {
            // Fallback: find button with "Reply" text in tweet element
            const buttons = await tweetEl.$$('button');
            for (const btn of buttons) {
              const text = await page.evaluate(el => el.textContent, btn);
              const name = await page.evaluate(el => el.getAttribute('name'), btn);
              if (text?.includes('Reply') || name === 'Reply') {
                replyBtn = btn;
                break;
              }
            }
